...
//    protected final Logger logger = LoggerFactory.getLogger(getClass());
   ...
   //         logger.error("Error in loadObject.", e);
    ...
    public List<T> find(String hql) {
        return find(hql, new String[0], new Object[0]);
    }
  ...
  //          logger.error("Error in find(String, Object[]).", e);
     ...
     * <pre>
     * exampl:
     * 	String hql = "select t from Admin t where t.store.id=:sid and t.type=:type";
        List<Admin> coll = find(hql, new String[] { "sid", "type" }, new Object[] { storeId, type });
     * </pre>
     * @return List of queried objects
     */
...
//                logger.debug("执行数据库查询用时:" + (System.currentTimeMillis() - begin));
//            }
        }
    }
    ...
    public List<T> find(String hql, String paraName, Object paraValue) {
        return find(hql, new String[] { paraName }, new Object[] { paraValue });
    }
    ...
    public List<T> find(String hql, int firstResult, int maxResults) {
        return find(hql, (String[])null, (Object[])null, firstResult, maxResults);
    }
...
//            }
    ...
    public List<T> find(String hql, Paginator paginator) {
        return find(hql, (String[]) null, (Object[]) null, paginator);
    }
    ...
    public List<T> find(final String hql, final Object[] paras, final Paginator paginator) {
        ...
        try {
            ...
            String orderBy = prepareOrderBy(paginator);
            Query queryObject;
            if (StringUtils.isNotBlank(orderBy)) {
                String orderHql = hql + orderBy;
    ...
    */
    ...
    public List<T> find(final String hql, final String[] paraNames, final Object[] paraValues, final Paginator paginator) {
        ...
        try {
            ...
            if (paginator.getTotalRecords() == null && (paginator.getStartIndex() != 0 || paginator.getPageSize() != Integer.MAX_VALUE)) {
                paginator.setTotalRecords(totalRecords(hql, paraNames, paraValues));
            }
            String orderBy = prepareOrderBy(paginator);
            Query queryObject;
            if (StringUtils.isNotBlank(orderBy)) {
...
//            }
    ...
    /**
     * calc paginator total records 
    * @param hql
    * @param paraNames
    * @param paraValues
    * @return
    */
    protected Integer totalRecords(String hql, String[] paraNames, Object[] paraValues) {
        ...
        return count instanceof Number ? ((Number) count).intValue() : Integer.MAX_VALUE;
    }
...
//            logger.error("Error in uniqueFind(String, Object[], int, int).", e);
 ...
 //           logger.error("Error in update(String, String[], Object[]).", e);
    ...
    protected String prepareOrderBy(Paginator paginator) {
        if (StringUtils.isNotBlank(paginator.getSort())) {
            StringBuffer buf = new StringBuffer(paginator.getSort());
            if (StringUtils.isNotBlank(paginator.getAlias())) {
                buf.insert(0, paginator.getAlias() + ".");
            }
            final String dir = StringUtils.trimToEmpty(paginator.getDir());
            buf.append(" ").append(StringUtils.isBlank(dir) ? "asc" : dir);
